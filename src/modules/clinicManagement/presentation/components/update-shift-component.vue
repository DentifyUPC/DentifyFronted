  ...
  <div class="fixed inset-0 z-[60] flex items-center justify-center bg-black bg-opacity-70 backdrop-blur-sm">
    <div class="modal-content relative">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        <div>
          <label class="block text-sm text-gray-600 mb-1">Nombre del Turno</label>
          <input
              :value="props.shift.displayName"
              disabled
...
onMounted(() => {
  form.value.startTime = props.shift.startTime || '';
  form.value.endTime = props.shift.endTime || '';
});
...
const handleSubmit = async () => {
  try {
    ...
    const updated = await shiftRepositoryImpl.update(props.shift.id, payload);
    alert('Turno actualizado exitosamente');
    emit('updated', updated);
  } catch (error) {
    console.error('Error al actualizar turno:', error);
    alert(error.response?.data?.error || 'Error al actualizar turno');
  } finally {
